...
if (!fse.existsSync(iconsDir))
{
  console.log(pc.bold(pc.red(
		`Couldn't read directory ${iconsDir}! In file ${path.basename(__filename)}`
	)));
  process.exit(1);
}
...
else
{
  console.log(pc.red(pc.bold(
		`Directory ${iconsDir} is empty! In file ${path.basename(__filename)}`
	)));
	process.exit(1);
}
...
module.exports.main = async () =>
{
  try
  {
    ...
    const timeLabel = pc.cyan(pc.bold(`[${basename}] finished`));

    await console.log(pc.cyan(pc.bold(`[${basename}] started`)));
    await console.time(timeLabel);
